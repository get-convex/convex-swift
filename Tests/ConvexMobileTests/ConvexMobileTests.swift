...
final class ConvexMobileTests: XCTestCase {
    func testSubscribe() async throws {
        ...
        let cancellationHandle = try await client.subscribe(name: "foo").sink(receiveCompletion: { completion in switch completion {
        case .finished:
            break
...
class FakeMobileConvexClient : UniFFI.MobileConvexClientProtocol {
    ...
    func subscribe(name: String, args: [String : String], subscriber: any UniFFI.QuerySubscriber) async throws -> UniFFI.SubscriptionHandle {
        let _ = Task {
